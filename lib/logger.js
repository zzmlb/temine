...
function separateCmdAndOutput(newLines) {
  ...
  for (let i = 0; i < newLines.length; i++) {
    ...
    for (const p of CMD_PATTERNS) {
      ...
      if (m) {
        const cmd = m[1].trim();
        if (cmd && !/^[-=─]+$/.test(cmd) && cmd.length < 300) {
          cmds.push(cmd);
          isCmd = true;
        }
        break;
...
/**
 * 行级去重：找出真正新增的行
 * 排除最后一行（当前正在输入的命令/提示符，还没回车）
 */
function findNewLines(prevLines, currLines) {
  // 排除最后一行（未提交的输入行）
  const curr = currLines.length > 1 ? currLines.slice(0, -1) : [];
  const prev = prevLines.length > 1 ? prevLines.slice(0, -1) : [];
  ...
  if (commonPrefix > 0) return curr.slice(commonPrefix);
  ...
  for (let i = curr.length - tailLen; i >= 0; i--) {
    ...
    if (match) return curr.slice(i + tailLen);
  }
...
export function recordSnapshots(windowsData) {
  ...
  for (const { windowId, windowName, content, label, skipCmd, skipOut } of windowsData) {
    ...
    if (newLines.length > 0 && newLines.some(l => l.trim())) {
      ...
      // 写入命令文件（去重：同一个命令只记录一次）
      if (cmds.length > 0 && !skipCmd) {
        let seenSet = _seenCmds.get(safeId);
        if (!seenSet) { seenSet = new Set(); _seenCmds.set(safeId, seenSet); }
        const newCmds = cmds.filter(c => !seenSet.has(c));
        if (newCmds.length > 0) {
          const cmdFile = join(LOGS_DIR, `${safeId}.cmd.log`);
          const entries = newCmds.map(c => `[${ts}] ${c}`).join('\n');
          try { appendFileSync(cmdFile, entries + '\n'); } catch {}
          for (const c of newCmds) seenSet.add(c);
          // 限制 set 大小，防止内存增长
          if (seenSet.size > 500) {
            const arr = [...seenSet];
            _seenCmds.set(safeId, new Set(arr.slice(-300)));
          }
        }
      }
  ...
  if (_markerCache.size > windowsData.length + 5) {
    ...
    for (const id of _markerCache.keys()) {
      if (!activeIds.has(id)) { _markerCache.delete(id); _seenCmds.delete(id); }
    }
  }
}
