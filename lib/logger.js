...
/**
 * 判断文本是否像真正的 shell 命令（而非自然语言/输出内容）
 * 真正的命令：cd, ls, git, npm, claude, temine, python ...
 * 非命令：中文聊天、游戏输出、长段落
 */
function looksLikeCommand(text) {
  // 太长的不是命令（shell 命令极少超过 150 字符）
  if (text.length > 150) return false;
  // 统计中文字符占比，超过 30% 认为是自然语言输入而非 shell 命令
  const cjk = text.match(/[\u4e00-\u9fff\u3400-\u4dbf]/g);
  if (cjk && cjk.length > text.length * 0.3) return false;
  // 不含任何字母或数字的不是命令（纯符号/中文）
  if (!/[a-zA-Z0-9]/.test(text)) return false;
  // 含有明显输出特征的排除
  if (/^(Boss|Error|Warning|Info|─|═|✓|✗|●|○)/i.test(text)) return false;
  // 含有省略号 + 括号等输出格式
