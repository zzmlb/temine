...
function stackChromeWindows(opts) {
  ...
  // 构造 osascript
  // 1) 设置每个窗口 bounds：x=0, y=i*reveal, width=screenW, height=screenH
  // 2) 调整 z-order：从 i=0 到 i=N-1 依次 set index to 1，最后处理的(i=N-1, 最底位置)显示在最上层
  let cmds = 'activate\n';
  ...
  for (let i = 0; i < N; i++) {
    ...
    const y = i * reveal;
    cmds += `try
  set bounds of (window id ${w.id}) to {0, ${y}, ${screenW}, ${y + screenH}}
end try
`;
  }
  for (let i = 0; i < N; i++) {
    const w = ordered[i];
    cmds += `try
  tell window id ${w.id} to set index to 1
end try
`;
  }
  try {
