...
import { labelWindow, getLabel } from './terminal-label.js';
...
function arrangeWindowsApi(cols, regionId) {
  ...
  try {
    const cnt = parseInt(execSync(`osascript -e 'tell application "Terminal" to return count of windows'`, {
...
<div class="topbar">
  ...
  <div class="tabs">
    ...
    <button class="tab-btn" data-tab="records" onclick="switchTab('records')">输出记录 <span id="recCount"></span></button>
  </div>
...
<div id="tabRecords" style="display:none">
  <div class="records-page">
    <h3>已保存的终端输出</h3>
    <div id="recordsList"><div class="empty"><h2>加载中...</h2></div></div>
  </div>
</div>
...
function render(d){
  ...
  cc.innerHTML=d.windows.map(function(w){
    ...
    return '<div class="card state-'+sc+'" data-si="'+si+'" data-winid="'+escA(w.id)+'" ondblclick="focusWin(this.dataset.winid)" title="双击跳转到此终端">' +
      '<div class="card-header"><div class="card-header-left"><span class="card-idx">'+(NUM[si-1]||si)+'</span>' +
        '<input class="card-label" value="'+lv+'" placeholder="'+ph+'" data-apiindex="'+w.index+'" data-stableidx="'+si+'" oninput="onLabelInput(this)" onblur="onLabelBlur(this)">' +
      ...
      '<div class="card-footer">' +
        '<button class="fbtn" data-winid="'+escA(w.id)+'" data-apiindex="'+w.index+'" data-name="'+escA(w.labelText||w.name||'窗口'+si)+'" data-stableidx="'+si+'" onclick="saveRecord(this)">📋 保存输出</button>' +
        '<button class="fbtn" data-winid="'+escA(w.id)+'" data-name="'+escA(w.labelText||w.name||'窗口'+si)+'" onclick="showHistory(this)">📝 命令历史</button>' +
...
function submitLabel(inp){
  ...
  fetch('/api/label',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({index:parseInt(inp.dataset.apiindex),label:label,stableIndex:parseInt(inp.dataset.stableidx)})
  }).catch(function(){});
}
...
function showHistory(btn){
  document.getElementById('historyTitle').textContent=(btn.dataset.name||'窗口')+' — 命令历史';
  document.getElementById('historyBody').innerHTML='<div style="color:#8b949e">加载中...</div>';
  document.getElementById('historyModal').style.display='flex';
  fetch('/api/history?id='+encodeURIComponent(btn.dataset.winid)).then(function(r){return r.json()}).then(function(d){
    var body=document.getElementById('historyBody');
    if(!d.commands||!d.commands.length){body.innerHTML='<div style="color:#484f58;text-align:center;padding:30px">暂无记录。需先运行 <code style="background:#21262d;padding:2px 6px;border-radius:4px;color:#58a6ff">temine watch</code></div>'}
    else{body.innerHTML=d.commands.map(function(c){return '<div class="cmd-line">'+esc(c)+'</div>'}).join('')}
  }).catch(function(){document.getElementById('historyBody').innerHTML='<div style="color:#f85149">加载失败</div>'});
}

// === 输出记录 Tab ===
function loadRecords(){
  fetch('/api/records').then(function(r){return r.json()}).then(function(d){
    var el=document.getElementById('recCount');
    el.textContent=d.records.length?'('+d.records.length+')':'';
    var list=document.getElementById('recordsList');
    if(!d.records.length){list.innerHTML='<div class="empty"><h2>暂无保存的记录</h2><p>在控制面板中点击"📋 保存输出"来捕获终端内容</p></div>';return}
...
function viewRecord(id){
  document.getElementById('recordTitle').textContent='输出记录';
...
function deleteRecord(id){
  ...
  fetch('/api/record?id='+id,{method:'DELETE'}).then(function(){loadRecords()}).catch(function(){});
}
...
export async function startPanel(port) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/label' && req.method === 'POST') {
      ...
      req.on('end', async () => {
        try {
          const { index, label, stableIndex } = JSON.parse(body);
    ...
    if (url.pathname === '/api/history' && req.method === 'GET') {
      ...
      let commands = [];
      try { commands = extractCommands(readFileSync(logFile, 'utf-8')).slice(-100); } catch {}
      res.writeHead(200, { 'Content-Type': 'application/json' });
