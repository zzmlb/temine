...
function launchSSHPresets(presetIds) {
  ...
  for (const pid of presetIds) {
    ...
    if (conn.type === 'local') {
      // 本地模式：直接打开终端执行命令
      // dir 已做单引号转义，用单引号包裹防止 shell 元字符注入
      let localCmd = '';
...
<div id="tabRecords" style="display:none">
  <div class="records-page">
    <div class="sub-tabs">
      ...
      <input class="log-search-input" id="logSearchInput" placeholder="搜索关键词..." onkeydown="if(event.key===\\'Enter\\')searchLogsUI()">
      <button class="btn" style="font-size:11px;padding:3px 10px" onclick="searchLogsUI()">搜索</button>
      <button class="btn" id="clearTypeBtn" style="font-size:11px;padding:3px 10px;color:#f85149;border-color:#f85149" onclick="clearLogsByTypeUI()">清空输出</button>
    </div>
    ...
    <div class="ssh-left">
      <div class="ssh-section-head">
        ...
        <div style="display:flex;gap:6px">
          <button class="btn" onclick="showConnForm(null,'local')">+ 本地</button>
          <button class="btn" onclick="showConnForm(null,'ssh')">+ SSH</button>
        </div>
      </div>
  ...
  function renderCard(w,idx){
    ...
    return '<div class="card state-'+sc+swapCls+'" data-si="'+si+'" data-winid="'+escA(w.id)+'" ondblclick="focusWin(this.dataset.winid)" title="双击跳转到此终端">' +
      ...
      '<div class="card-footer">' +
        (globalRecEnabled?
        '<span class="rec-toggle '+(w.recCmd?'on':'off')+'" title="命令录制：开启后自动记录此终端执行的命令到日志" onclick="event.stopPropagation();toggleRec(\\''+escA(w.id)+'\\',\\'cmd\\')">'+(w.recCmd?'● 命令':'○ 命令')+'</span>' +
        '<span class="rec-toggle '+(w.recOut?'on':'off')+'" title="输出录制：开启后自动记录此终端的输出内容到日志" onclick="event.stopPropagation();toggleRec(\\''+escA(w.id)+'\\',\\'out\\')">'+(w.recOut?'● 输出':'○ 输出')+'</span>'
        :
        ...
        '<button class="fbtn" title="换位：点击后再选另一终端，两者屏幕位置互换" onclick="event.stopPropagation();startSwap(\\''+escA(w.id)+'\\')">⇄ 换位</button>' +
...
function updateGlobalRecUI(enabled){
  ...
  if(enabled){b.className='global-rec on';b.innerHTML='● 录制中';b.title='全局录制开关：当前已开启，点击暂停所有录制'}
  else{b.className='global-rec off';b.innerHTML='⏸ 已暂停';b.title='全局录制开关：当前已暂停，点击恢复录制'}
}
...
function renderConns(){
  ...
  list.innerHTML=ids.map(function(id){
    ...
    var dotCls=isLocal?'ok':(c.lastTestResult===true?'ok':(c.lastTestResult===false?'fail':'unknown'));
    var hostLine=isLocal?'本地终端':(esc(c.username||'')+'@'+esc(c.host||'')+':'+esc(String(c.port||22)));
    var testBtn=isLocal?'':'<button class="btn" style="font-size:11px;padding:2px 8px" onclick="event.stopPropagation();testConn(\\''+escA(id)+'\\',this)">测试</button>';
...
function selectConn(id){
  ...
  var c=sshConns[id];
  document.getElementById('presetConnName').textContent=c?(c.type==='local'?c.name+' (本地)':(c.username+'@'+c.host)):'';
  loadPresets(id);
}
...
function showConnForm(editId,type){
  ...
  document.getElementById('sshFields').style.display=t==='local'?'none':'';
      ...
      readBody(req, (body) => {
        try {
          ...
          if (connType === 'local') {
            ...
            conns[id] = { id, type: 'ssh', name: d.name, host: d.host, port, username: d.username, keyPath: d.keyPath || '', lastTestResult: null };
          }
      ...
      readBody(req, (body) => {
        try {
          ...
          if (connType === 'local') {
            ...
            Object.assign(conns[d.id], { name: d.name || conns[d.id].name, type: 'ssh', host: d.host || conns[d.id].host, port: parseInt(d.port) || conns[d.id].port, username: d.username || conns[d.id].username, keyPath: newKeyPath });
          }
          saveSSHConnections(conns);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true }));
