...
const ISLAND_CONFIG_FILE = join(STATE_DIR, 'island-config.json');
...
const DEFAULT_ISLAND_CONFIG = { arrangeSequence: ['full', 'top-half', 'bottom-half'], animationEnabled: true };
const ALL_ARRANGE_REGIONS = [
  { id: 'full', name: '全屏' },
  { id: 'top-half', name: '上 1/2' },
  { id: 'top-2-3', name: '上 2/3' },
  { id: 'bottom-half', name: '下 1/2' },
  { id: 'left-half', name: '左 1/2' },
  { id: 'right-half', name: '右 1/2' },
  { id: 'left-2-3', name: '左 2/3' },
  { id: 'left-1-3', name: '左 1/3' },
];
function loadIslandConfig() {
  try { return JSON.parse(readFileSync(ISLAND_CONFIG_FILE, 'utf-8')); } catch { return DEFAULT_ISLAND_CONFIG; }
}
function saveIslandConfig(c) { ensureDir(); writeFileSync(ISLAND_CONFIG_FILE, JSON.stringify(c, null, 2)); }
...
function stackChromeWindows(opts) {
  ...
  const reveal = parseInt(o.reveal) || 100;     // 默认 100px：足够露出完整 tab bar + 一点地址栏
  const yOffset = parseInt(o.yOffset) || 28;     // 避开 macOS 菜单栏 24px
  if (screenW < 200 || screenH < 200 || reveal < 30 || reveal > 400) return false;
  ...
  // 算法：窗口高度 = 屏幕高度（不压缩），y 错开 reveal，底部超出屏幕没关系
  // 这样保证每个窗口都有完整 Chrome chrome（tab bar + 地址栏可见在 reveal 区域内）
  // y 起点 yOffset (28) 避开 macOS 菜单栏
  const N = ordered.length;
  const winHeight = screenH;  // 不压缩，每个窗口高度都是屏幕高度
  let cmds = 'activate\ndelay 0.05\n';
  for (let i = 0; i < N; i++) {
    const w = ordered[i];
    const y = yOffset + i * reveal;
...
`;
  ...
  // z-order: ordered[N-1] (最底 y=最大) → index=1 最前完整可见
  //          ordered[0]   (最顶 y=yOffset) → index=N 最后只露 reveal 像素 tab bar
  // 反向遍历：先 set 最底层（N-1）为 1，再 set 倒数第二个（N-2）为 1（覆盖）
  // 等价于一次性设 index = N - i，加 5ms delay 让 Chrome 真正响应每次设置
  for (let i = 0; i < N; i++) {
    const w = ordered[i];
    const idx = N - i;
    cmds += `try
...
.update-btn,.theme-toggle,.watcher-toggle{background:#21262d;border:1px solid #30363d;color:#c9d1d9;padding:5px 12px;border-radius:6px;font-size:12px;cursor:pointer;transition:all .15s;white-space:nowrap}
.update-btn:hover,.theme-toggle:hover,.watcher-toggle:hover{background:#30363d;border-color:#58a6ff}
.watcher-toggle.on{background:#0d4429;color:#3fb950;border-color:#238636}
.watcher-toggle.on:hover{background:#0f5435;border-color:#3fb950}
...
/* === 灵动岛配置 tab === */
.arrange-cfg-item{display:flex;align-items:center;gap:10px;padding:10px 12px;background:#161b22;border:1px solid #30363d;border-radius:6px;margin-bottom:6px}
.arrange-cfg-item input[type=checkbox]{width:auto;margin:0;cursor:pointer;flex-shrink:0}
.arrange-cfg-item .arrange-cfg-name{flex:1;color:#c9d1d9;font-size:13px}
.arrange-cfg-item .arrange-cfg-pos{color:#8b949e;font-size:11px;margin-right:8px}
.arrange-cfg-item .arrange-cfg-move{background:#21262d;border:1px solid #30363d;color:#c9d1d9;padding:2px 8px;border-radius:4px;cursor:pointer;font-size:11px}
.arrange-cfg-item .arrange-cfg-move:hover{border-color:#58a6ff}
.arrange-cfg-item .arrange-cfg-move:disabled{opacity:0.3;cursor:not-allowed}
body.theme-warp .arrange-cfg-item{background:rgba(28,20,40,0.55);border-color:rgba(255,138,200,0.18)}
...
<div class="topbar">
  ...
  <div class="tabs">
    ...
    <button class="tab-btn" data-tab="islandcfg" onclick="switchTab('islandcfg')">灵动岛配置</button>
  </div>
  <div class="topbar-right">
    ...
    <button class="watcher-toggle" id="watcherToggleBtn" onclick="toggleWatcher()" title="AI 状态监控（默认关闭，开启后每秒读 Terminal 内容，可能触发 macOS Terminal 崩溃）">🔍 监控</button>
...
<!-- ====== TAB: 灵动岛配置 ====== -->
<div id="tabIslandCfg" style="display:none">
  <div style="padding:16px 24px;max-width:720px">
    <div style="margin-bottom:24px;padding-bottom:16px;border-bottom:1px solid #30363d">
      <h3 style="font-size:14px;color:#c9d1d9;margin-bottom:8px">动画开关</h3>
      <label style="display:flex;align-items:center;gap:8px;cursor:pointer;color:#c9d1d9;font-size:13px">
        <input type="checkbox" id="cfgAnimationEnabled" style="width:auto;margin:0;cursor:pointer">
        <span>开启紧凑态动画（光点呼吸 + 山洞探险剧情）</span>
      </label>
      <p style="margin-top:6px;color:#8b949e;font-size:11px;line-height:1.5">
        关闭后灵动岛只显示静态光点，不创建舞台、不渲染角色，省 GPU/电池。
        修改后需重启灵动岛：<code>temine island stop &amp;&amp; temine island</code>
      </p>
    </div>
    <h3 style="font-size:14px;color:#c9d1d9;margin-bottom:8px">排版按钮循环序列</h3>
    <p class="section-desc" style="margin-bottom:16px">勾选要激活的排版方式，灵动岛上的「排版」按钮会按列表顺序循环：激活 2 个就两次循环，激活 5 个就五次循环。可以用 ↑↓ 调整顺序。</p>
    <div id="arrangeCfgList" style="margin-bottom:16px"></div>
    <div style="display:flex;gap:8px;align-items:center">
      <button class="btn primary" onclick="saveIslandCfg()">保存</button>
      <button class="btn" onclick="resetIslandCfg()">恢复默认</button>
      <span id="islandCfgMsg" style="color:#3fb950;font-size:12px;margin-left:8px"></span>
    </div>
  </div>
</div>
...
// ── AI 监控开关（默认关，避免触发 Terminal NSPersistentUI 崩溃）──
function refreshWatcherBtn(){
  fetch('/api/watcher/status').then(function(r){return r.json()}).then(function(d){
    var btn=document.getElementById('watcherToggleBtn');
    if(!btn)return;
    if(d.running){btn.classList.add('on');btn.textContent='🔍 监控中';}
    else{btn.classList.remove('on');btn.textContent='🔍 监控';}
  }).catch(function(){});
}
function toggleWatcher(){
  var btn=document.getElementById('watcherToggleBtn');
  if(!btn)return;
  var running=btn.classList.contains('on');
  var url=running?'/api/watcher/stop':'/api/watcher/start';
  btn.disabled=true;
...
// 启动时 + 每 5 秒查一次（监控可能被外部停止）
setTimeout(refreshWatcherBtn,500);
setInterval(refreshWatcherBtn,5000);
...
function switchTab(t){
  ...
  document.getElementById('tabIslandCfg').style.display=t==='islandcfg'?'':'none';
  ...
  if(t==='islandcfg')loadIslandCfg();
}

// === 灵动岛配置 tab ===
var ALL_ARRANGE_REGIONS_CLIENT=[
  {id:'full',name:'全屏'},
  {id:'top-half',name:'上 1/2'},
  {id:'top-2-3',name:'上 2/3'},
  {id:'bottom-half',name:'下 1/2'},
...
var islandCfgState={arrangeSequence:['full','top-half','bottom-half'],animationEnabled:true};
function loadIslandCfg(){
  fetch('/api/island/config').then(function(r){return r.json()}).then(function(c){
    if(c&&Array.isArray(c.arrangeSequence)&&c.arrangeSequence.length>0)islandCfgState.arrangeSequence=c.arrangeSequence;
    islandCfgState.animationEnabled=typeof c.animationEnabled==='boolean'?c.animationEnabled:true;
    renderIslandCfg();
  }).catch(function(){renderIslandCfg()});
}
function renderIslandCfg(){
  var animChk=document.getElementById('cfgAnimationEnabled');
  if(animChk)animChk.checked=!!islandCfgState.animationEnabled;
  var list=document.getElementById('arrangeCfgList');
  if(!list)return;
  var active=islandCfgState.arrangeSequence||[];
  var activeSet={};active.forEach(function(id,i){activeSet[id]=i});
  // 先列已激活的（按当前顺序，可调整）
  var html='';
  active.forEach(function(id,i){
    var meta=ALL_ARRANGE_REGIONS_CLIENT.find(function(x){return x.id===id});
    if(!meta)return;
    var upDisabled=(i===0)?' disabled':'';
    var downDisabled=(i===active.length-1)?' disabled':'';
    html+='<div class="arrange-cfg-item">'+
      '<input type="checkbox" checked onchange="toggleArrangeRegion(\\''+id+'\\',this.checked)">'+
      '<span class="arrange-cfg-pos">'+(i+1)+'.</span>'+
      '<span class="arrange-cfg-name">'+esc(meta.name)+'</span>'+
      '<button class="arrange-cfg-move" onclick="moveArrangeRegion(\\''+id+'\\',-1)"'+upDisabled+'>↑</button>'+
      '<button class="arrange-cfg-move" onclick="moveArrangeRegion(\\''+id+'\\',1)"'+downDisabled+'>↓</button>'+
    '</div>';
  });
  // 再列未激活的
  ALL_ARRANGE_REGIONS_CLIENT.forEach(function(meta){
    if(activeSet[meta.id]===undefined){
      html+='<div class="arrange-cfg-item">'+
        '<input type="checkbox" onchange="toggleArrangeRegion(\\''+meta.id+'\\',this.checked)">'+
        '<span class="arrange-cfg-pos" style="visibility:hidden">-</span>'+
        '<span class="arrange-cfg-name" style="color:#6e7681">'+esc(meta.name)+'</span>'+
      '</div>';
    }
  });
  list.innerHTML=html;
}
function toggleArrangeRegion(id,on){
  var seq=islandCfgState.arrangeSequence||[];
  if(on){
    ...
    seq=seq.filter(function(x){return x!==id});
  }
  islandCfgState.arrangeSequence=seq;
  renderIslandCfg();
}
function moveArrangeRegion(id,delta){
  var seq=islandCfgState.arrangeSequence||[];
...
function saveIslandCfg(){
  var animChk=document.getElementById('cfgAnimationEnabled');
  if(animChk)islandCfgState.animationEnabled=!!animChk.checked;
  fetch('/api/island/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(islandCfgState)}).then(function(r){return r.json()}).then(function(d){
    var msg=document.getElementById('islandCfgMsg');
    if(d.ok){
      msg.textContent='✓ 已保存（灵动岛下次按按钮会用新配置）';
    ...
    var msg=document.getElementById('islandCfgMsg');
    msg.style.color='#f85149';msg.textContent='保存失败';
  });
}
function resetIslandCfg(){
  islandCfgState={arrangeSequence:['full','top-half','bottom-half']};
  renderIslandCfg();
}
...
export async function startPanel(port) {
  ...
  // AI 监控默认不自动启动（频繁 osascript 会触发 macOS Terminal NSPersistentUI 崩溃）
  // 用户在面板里手动启动：顶部「🔍 启动监控」按钮，或命令行 `temine watch`
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/watcher/status' && req.method === 'GET') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ running: !!isWatcherRunning() }));
      return;
    }
    // 启动监控
    if (url.pathname === '/api/watcher/start' && req.method === 'POST') {
      if (isWatcherRunning()) {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: true, running: true, alreadyRunning: true }));
        return;
      }
      try {
        ...
        watcher.unref();
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: true, running: true, pid: watcher.pid }));
      } catch (e) {
    ...
    // 灵动岛配置：GET 当前配置 / POST 保存
    if (url.pathname === '/api/island/config' && req.method === 'GET') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify(loadIslandConfig()));
      return;
    }
    if (url.pathname === '/api/island/config' && req.method === 'POST') {
      readBody(req, (body) => {
        try {
          const cfg = JSON.parse(body);
          if (!Array.isArray(cfg.arrangeSequence)) {
            res.writeHead(400); res.end('arrangeSequence must be array'); return;
          }
          const validIds = ALL_ARRANGE_REGIONS.map(x => x.id);
          const filtered = cfg.arrangeSequence.filter(id => validIds.includes(id));
          if (filtered.length === 0) {
            res.writeHead(400); res.end('at least one region required'); return;
          }
          const animationEnabled = typeof cfg.animationEnabled === 'boolean' ? cfg.animationEnabled : true;
          const saved = { arrangeSequence: filtered, animationEnabled };
          saveIslandConfig(saved);
          res.writeHead(200, { 'Content-Type': 'application/json' });
