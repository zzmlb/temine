...
function launchSSHPresets(presetIds) {
  ...
  for (const pid of presetIds) {
    ...
    if (conn.type === 'local' || conn.type === 'shell') {
      // shell 模式：直接执行 preset.command（不拼 ssh 用户名@主机）
      // local 模式：本机打开终端，可叠加 cd dir
      let localCmd = '';
...
function renderConns(){
  ...
  list.innerHTML=ids.map(function(id){
    ...
    var hostLine=isShell?'强制命令行':(isLocal?'本地终端':(esc(c.username||'')+'@'+esc(c.host||'')+':'+esc(String(c.port||22))));
