...
const DEFAULT_ISLAND_CONFIG = { arrangeSequence: ['full', 'top-half', 'bottom-half'] };
...
var islandCfgState={arrangeSequence:['full','top-half','bottom-half']};
function loadIslandCfg(){
  fetch('/api/island/config').then(function(r){return r.json()}).then(function(c){
    if(c&&Array.isArray(c.arrangeSequence)&&c.arrangeSequence.length>0)islandCfgState=c;
    renderIslandCfg();
    ...
    if (url.pathname === '/api/island/config' && req.method === 'POST') {
      readBody(req, (body) => {
        try {
          ...
          saveIslandConfig({ arrangeSequence: filtered });
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true, arrangeSequence: filtered }));
        } catch {
          res.writeHead(400); res.end('bad request');
        }
      });
