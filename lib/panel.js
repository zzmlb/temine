...
import { clearAllLogs } from './logger.js';
...
function launchSSHPresets(presetIds) {
  ...
  for (const pid of presetIds) {
    ...
    const fullCmd = `${sshCmd} ${safeUser}@${safeHost}${remoteCmd}`;
    try {
      // AppleScript 转义：先转义 \，再转义 "（顺序重要）
  ...
  if (launched.length > 0) {
    setTimeout(() => { try { arrangeWindowsApi(0, 'full'); } catch {} }, 2000);
  }
...
.card.swap-select{box-shadow:0 0 0 2px #58a6ff;border-color:#58a6ff}
.swap-hint{background:#0c2d6b;color:#58a6ff;padding:6px 16px;font-size:12px;border-radius:6px;display:none;align-items:center;gap:8px}
.swap-hint.show{display:inline-flex}
.swap-hint .btn{font-size:11px;padding:2px 10px}
...
<div class="topbar">
  ...
  <div class="topbar-right">
    <span class="swap-hint" id="swapHint">选择第二个窗口以互换位置 <button class="btn" onclick="cancelSwap()">取消</button></span>
    <span class="global-rec on" id="globalRecBadge" onclick="toggleGlobalRec()" title="全局录制开关">● 录制中</span>
...
<div id="tabRecords" style="display:none">
  <div class="records-page">
    <div class="sub-tabs">
      ...
      <button class="btn" style="font-size:11px;padding:3px 10px;color:#f85149;border-color:#f85149" onclick="clearAllLogs()">清空所有日志</button>
    </div>
...
<div id="tabSSH" style="display:none">
  <div class="ssh-container">
    <div class="ssh-left">
      <div class="ssh-section-head">
        <h3>SSH 连接</h3>
        <button class="btn" onclick="showConnForm()">+ 新建连接</button>
      </div>
      ...
      <div id="connList"><div class="empty" style="padding:20px"><h2>暂无连接</h2><p>点击上方"+ 新建连接"添加</p></div></div>
    </div>
    <div class="ssh-right">
      <div id="presetHeader" style="display:none">
        ...
        <div id="presetForm" style="display:none">
          <div class="ssh-form">
            ...
            <label>启动命令</label>
            <input id="pfCmd" placeholder="claude" value="claude">
      ...
      <div id="presetEmpty"><div class="empty" style="padding:40px 20px"><h2>请选择左侧的 SSH 连接</h2><p>选择连接后可管理其项目预设</p></div></div>
    </div>
  </div>
  <div class="ssh-launch-bar" id="sshLaunchBar" style="display:none">
    ...
    <button class="btn primary" onclick="launchSelected()">一键打开</button>
  </div>
</div>
...
var NUM=['①','②','③','④','⑤','⑥','⑦','⑧','⑨','⑩'];
var data=null,selectedLayout=null,selectedRegion='full',swapFirst=null;
...
function render(d){
  ...
  function renderCard(w,idx){
    ...
    var swapCls=(swapFirst===w.id)?' swap-select':'';
    return '<div class="card state-'+sc+swapCls+'" data-si="'+si+'" data-winid="'+escA(w.id)+'" ondblclick="focusWin(this.dataset.winid)" title="双击跳转到此终端">' +
      ...
      '<div class="card-footer">' +
        '<span class="rec-toggle '+(w.recCmd?'on':'off')+'" onclick="event.stopPropagation();toggleRec(\\''+escA(w.id)+'\\',\\'cmd\\')">'+(w.recCmd?'● 命令':'○ 命令')+'</span>' +
        '<span class="rec-toggle '+(w.recOut?'on':'off')+'" onclick="event.stopPropagation();toggleRec(\\''+escA(w.id)+'\\',\\'out\\')">'+(w.recOut?'● 输出':'○ 输出')+'</span>' +
        '<button class="fbtn" data-winid="'+escA(w.id)+'" data-apiindex="'+w.index+'" data-name="'+escA(w.labelText||w.name||'窗口'+si)+'" data-stableidx="'+si+'" onclick="event.stopPropagation();saveRecord(this)">📋 快照</button>' +
        (w.ignored?'<button class="fbtn ignore-on" onclick="event.stopPropagation();toggleIgnore(\\''+escA(w.id)+'\\',true)">👁 显示</button>':
        '<button class="fbtn" onclick="event.stopPropagation();startSwap(\\''+escA(w.id)+'\\')">⇄ 换位</button>' +
        '<button class="fbtn" onclick="event.stopPropagation();toggleIgnore(\\''+escA(w.id)+'\\',false)">🙈 忽略</button>') +
...
function startSwap(winId){
  if(swapFirst===null){
    swapFirst=winId;
    document.getElementById('swapHint').classList.add('show');
    // 给所有非忽略卡片加点击事件
    document.querySelectorAll('#cards .card').forEach(function(c){
      c.style.cursor='pointer';
      c.onclick=function(){
        var target=c.dataset.winid;
        if(target&&target!==swapFirst){doSwap(swapFirst,target)}
      };
    });
    if(data)render(data); // 刷新高亮
  }else{
    doSwap(swapFirst,winId);
  }
}
function doSwap(a,b){
  cancelSwap();
  fetch('/api/swap',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({winIdA:a,winIdB:b})})
    .then(function(r){return r.json()}).then(function(){_statusCache=null;fetchData()}).catch(function(){});
}
function cancelSwap(){
  swapFirst=null;
  document.getElementById('swapHint').classList.remove('show');
  document.querySelectorAll('#cards .card').forEach(function(c){c.style.cursor='';c.onclick=null});
  if(data)render(data);
}
...
function updateGlobalRecUI(enabled){
  ...
  if(enabled){b.className='global-rec on';b.innerHTML='● 录制中'}
  else{b.className='global-rec off';b.innerHTML='⏸ 已暂停'}
}
...
function renderConns(){
  ...
  list.innerHTML=ids.map(function(id){
    ...
    var dotCls=c.lastTestResult===true?'ok':(c.lastTestResult===false?'fail':'unknown');
    return '<div class="conn-item'+active+'" onclick="selectConn(\\''+escA(id)+'\\')"><div class="conn-name">'+esc(c.name)+'<span class="conn-dot '+dotCls+'"></span></div><div class="conn-host">'+esc(c.username||'')+'@'+esc(c.host||'')+':'+esc(String(c.port||22))+'</div><div class="conn-actions"><button class="btn" style="font-size:11px;padding:2px 8px" onclick="event.stopPropagation();editConn(\\''+escA(id)+'\\')">编辑</button><button class="btn" style="font-size:11px;padding:2px 8px;color:#f85149" onclick="event.stopPropagation();deleteConn(\\''+escA(id)+'\\')">删除</button><button class="btn" style="font-size:11px;padding:2px 8px" onclick="event.stopPropagation();testConn(\\''+escA(id)+'\\',this)">测试</button></div></div>';
  }).join('');
}
function selectConn(id){
  ...
  var c=sshConns[id];
  document.getElementById('presetConnName').textContent=c?(c.username+'@'+c.host):'';
  loadPresets(id);
}
...
function renderPresets(){
  ...
  if(!ids.length){list.innerHTML='<div style="color:#484f58;text-align:center;padding:20px;font-size:13px">暂无预设，点击"+ 新建预设"添加</div>';updateLaunchBar();return}
  list.innerHTML=ids.map(function(id){
    var p=sshPresets[id];
    ...
    return '<div class="preset-item"><input type="checkbox" class="preset-check"'+checked+' onchange="togglePresetSelect(\\''+escA(id)+'\\',this.checked)"><div class="preset-info"><div class="preset-name">'+esc(p.name)+'</div><div class="preset-cmd">'+cmdStr+'</div><div class="preset-actions"><button class="btn" style="font-size:11px;padding:2px 8px" onclick="editPreset(\\''+escA(id)+'\\')">编辑</button><button class="btn" style="font-size:11px;padding:2px 8px;color:#f85149" onclick="deletePreset(\\''+escA(id)+'\\')">删除</button></div></div></div>';
  }).join('');
  updateLaunchBar();
}
...
function showConnForm(editId){
  document.getElementById('connForm').style.display='';
  if(!editId){document.getElementById('cfName').value='';document.getElementById('cfHost').value='';document.getElementById('cfPort').value='22';document.getElementById('cfUser').value='';document.getElementById('cfKeyPath').value='';document.getElementById('cfEditId').value=''}
}
...
function saveConn(){
  var d={name:document.getElementById('cfName').value.trim(),host:document.getElementById('cfHost').value.trim(),port:parseInt(document.getElementById('cfPort').value)||22,username:document.getElementById('cfUser').value.trim(),keyPath:document.getElementById('cfKeyPath').value.trim()};
...
function showPresetForm(editId){
  ...
  if(!editId){document.getElementById('pfName').value='';document.getElementById('pfDir').value='';document.getElementById('pfCmd').value='claude';document.getElementById('pfEditId').value=''}
}
...
function savePreset(){
  var d={name:document.getElementById('pfName').value.trim(),directory:document.getElementById('pfDir').value.trim(),command:document.getElementById('pfCmd').value.trim(),connectionId:selectedConnId};
...
function launchSelected(){
  ...
  fetch('/api/ssh/launch',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({presetIds:Array.from(selectedPresetIds)})}).then(function(r){return r.json()}).then(function(d){
    if(d.ok){btn.textContent='✓ 已启动 '+d.launched.length+' 个';setTimeout(function(){btn.textContent='一键打开';btn.disabled=false},2000)}
...
export async function startPanel(port) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname === '/api/ssh/connections' && req.method === 'GET') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify(loadSSHConnections())); return;
    }
    ...
    if (url.pathname === '/api/ssh/connections' && req.method === 'POST') {
      readBody(req, (body) => {
        try {
          ...
          if (!d.name || !d.host || !d.username) { res.writeHead(400); res.end('missing fields'); return; }
          ...
          conns[id] = { id, name: d.name, host: d.host, port, username: d.username, keyPath: d.keyPath || '', lastTestResult: null };
    ...
    if (url.pathname === '/api/ssh/connections' && req.method === 'PUT') {
      readBody(req, (body) => {
        try {
          ...
          Object.assign(conns[d.id], { name: d.name || conns[d.id].name, host: d.host || conns[d.id].host, port: parseInt(d.port) || conns[d.id].port, username: d.username || conns[d.id].username, keyPath: d.keyPath !== undefined ? d.keyPath : conns[d.id].keyPath });
          saveSSHConnections(conns);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true }));
        } catch { res.writeHead(400); res.end('bad request'); }
    ...
    if (url.pathname === '/api/ssh/presets' && req.method === 'POST') {
      readBody(req, (body) => {
        try {
          ...
          presets[id] = { id, connectionId: d.connectionId, name: d.name, directory: d.directory || '', command: d.command || '' };
    ...
    if (url.pathname === '/api/ssh/presets' && req.method === 'PUT') {
      readBody(req, (body) => {
        try {
          ...
          Object.assign(presets[d.id], { name: d.name || presets[d.id].name, directory: d.directory !== undefined ? d.directory : presets[d.id].directory, command: d.command !== undefined ? d.command : presets[d.id].command });
          saveSSHPresets(presets);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true }));
        } catch { res.writeHead(400); res.end('bad request'); }
