...
function launchSSHPresets(presetIds) {
  ...
  for (const pid of presetIds) {
    ...
    // 强制命令模式：连接 type 是 'shell'，或 SSH 连接勾了 forceCommand 复选框
    // → 都不走 ssh user@host 拼接，直接在本地终端执行 preset.command
    const isForceCommand = conn.type === 'shell' || conn.forceCommand === true;

    if (conn.type === 'local' || isForceCommand) {
      let localCmd = '';
      ...
      <div id="connForm" style="display:none">
        <div class="ssh-form">
          ...
          <div id="sshFields">
            ...
            <label style="display:flex;align-items:center;gap:6px;margin-top:10px;cursor:pointer;font-size:12px;color:#c9d1d9;font-weight:500">
              <input type="checkbox" id="cfForceCommand" style="width:auto;margin:0;cursor:pointer">
              <span>强制命令模式（启动时直接执行预设命令，不拼接 ssh user@host）</span>
            </label>
          </div>
...
function renderConns(){
  ...
  list.innerHTML=ids.map(function(id){
    ...
    var isForce=(c.type==='ssh' && c.forceCommand===true);
    var noNetwork=isLocal||isShell||isForce;
    var dotCls=noNetwork?'ok':(c.lastTestResult===true?'ok':(c.lastTestResult===false?'fail':'unknown'));
    var hostLine=isShell?'强制命令行'
      :(isLocal?'本地终端'
      :((isForce?'⚡ 强制命令 · ':'')+esc(c.username||'')+'@'+esc(c.host||'')+':'+esc(String(c.port||22))));
...
function showConnForm(editId,type){
  ...
  if(!editId){
    ...
    document.getElementById('cfForceCommand').checked=false;
...
function editConn(id){
  ...
  document.getElementById('cfForceCommand').checked=!!c.forceCommand;
...
function saveConn(){
  ...
  if(connType==='ssh'){
    ...
    d.forceCommand=document.getElementById('cfForceCommand').checked;
        ...
        try {
          ...
          if (connType === 'local') {
            ...
            Object.assign(conns[d.id], {
              ...
              keyPath: newKeyPath,
              forceCommand: typeof d.forceCommand === 'boolean' ? d.forceCommand : !!conns[d.id].forceCommand,
            });
          }
          saveSSHConnections(conns);
          res.writeHead(200, { 'Content-Type': 'application/json' });
