...
function loadRecordingSettings() {
  try { return JSON.parse(readFileSync(RECORDING_FILE, 'utf-8')); } catch { return {}; }
}
...
async function watchLoop() {
  ...
  const tick = () => {
    ...
    try {
      ...
      const snapshotBatch = [];
      const recSettings = loadRecordingSettings();
      ...
      for (const win of allWindows) {
        ...
        // 收集快照（稍后批量写入，按录制设置决定是否跳过命令/输出）
        const rs = recSettings[win.id] || {};
        snapshotBatch.push({
          windowId: win.id, windowName: win.name, content: win.content, label: customLabel,
          skipCmd: rs.cmd === false, skipOut: rs.out === false,
        });
      ...
      // 清理已关闭窗口的缓存（每 20 轮，约 60 秒一次）
      enforceTick++;
      if (enforceTick % 20 === 0) {
        const activeIds = new Set(allWindows.map(w => w.id));
        for (const id of previousStates.keys()) {
          if (!activeIds.has(id)) { previousStates.delete(id); lastNotifyTime.delete(id); waitingConfirmSince.delete(id); }
        }
      }

      // 每 10 轮强制恢复标签（约 30 秒一次，防止 shell 覆盖自定义标题）
